...
# Configuration
# -----------------------------------------------------------------------------
alpha = parse(Float64, get(ENV, "VMC_H1D_ALPHA", "1.0"))

# Defaults are tuned for the repository's singular 1D hydrogen VMC comparison.
targetN = parse(Int, get(ENV, "VMC_H1D_TARGETN", "50000"))
dt = parse(Float64, get(ENV, "VMC_H1D_DT", "0.01"))
nsteps = parse(Int, get(ENV, "VMC_H1D_NSTEPS", "1000"))

nbins = parse(Int, get(ENV, "VMC_H1D_NBINS", "220"))
smoothing_window = parse(Int, get(ENV, "VMC_H1D_SMOOTH_WINDOW", "9"))
fps = parse(Int, get(ENV, "VMC_H1D_GIF_FPS", "5"))
output_dir = joinpath(dirname(@__DIR__), "outputs", "animations")
mkpath(output_dir)
output_file = joinpath(output_dir, get(ENV, "VMC_H1D_GIF_NAME", "vmc_h1d_proposal_compare.gif"))

# Expand animation x-limits at least to this range for stable overlays.
xmin_hint = parse(Float64, get(ENV, "VMC_H1D_XMIN", "-8.0"))
xmax_hint = parse(Float64, get(ENV, "VMC_H1D_XMAX", "8.0"))
...
function _safe_init_x(rng; x_min::Float64=1e-3)
    x = rand(rng) + 5
    if abs(x) < x_min
        x = (rand(rng) < 0.5 ? -1.0 : 1.0) * x_min
    end
    return x
end
...
base_positions = [[_safe_init_x(rng_init)] for _ in 1:targetN]
...
target_density(x) = 2 * alpha^3 * x^2 * exp(-2 * alpha * abs(x))
xref = range(xlo, xhi; length=1000)
...
anim = @animate for i in 1:nframes
    ...
    hline!(p_mean_d, [-0.5]; color=:black, linestyle=:dash, linewidth=1.7, label="analytic -1/2")
    ...
    hline!(p_mean_g, [-0.5]; color=:black, linestyle=:dash, linewidth=1.7, label="analytic -1/2")
...
println("Saved 1D hydrogen VMC proposal-comparison animation: ", output_file)
